...
OUT_FILE="$OUT_DIR/piano_hand_size_part2_rough_cut_v10.mp4"
WORK_DIR="$(mktemp -d /private/tmp/piano-hand-size-rough-cut-v10.XXXXXX)"
...
finish_montage_with_vo_and_music() {
  ...
  local label="$2"
  local silent_video="$MONTAGE_DIR/silent_video.mp4"
  local out montage_duration fade_out_start vo_duration video_duration target_duration
  set_next_out
  out="$NEXT_OUT"
  ...
  # full sentence can finish. Add 0.4s of breathing room either way.
  ...
  target_duration="$(awk -v v="$video_duration" -v a="$vo_duration" 'BEGIN { m=(v>a?v:a); printf "%.3f", m + 0.4 }')"
  ...
  ffmpeg -y -hide_banner -loglevel error \
    ...
    -filter_complex "[1:a]aresample=${SR},loudnorm=I=-16:LRA=11:TP=-1.5,apad=whole_dur=${montage_duration}[vo];[2:a]aresample=${SR},volume=${MUSIC_UNDER_VO_VOLUME},atrim=0:${montage_duration},afade=t=in:st=0:d=0.8,afade=t=out:st=${fade_out_start}:d=1.2[music];[vo][music]amix=inputs=2:duration=first:dropout_transition=0[a]" \
    -map 0:v:0 -map "[a]" \
    -c:v copy \
    -c:a aac -b:a 160k -ar "$SR" -ac 2 \
    -t "$montage_duration" \
...
add_video "footage/08_Pickups_To_Record/055_PICKUP_front_facing_intro.MOV" 0 7.5
add_video "footage/08_Pickups_To_Record/055_PICKUP_front_facing_intro.MOV" 13.5 18.5
...
add_video "footage/01_Trip_Setup/001_IMG_0256_0142am_trip_setup.MOV" 0 17.5
...
finish_montage_with_vo_and_music "audio/voiceovers/VO_01a_late_night_thesis.wav" "VO 01a thesis"

# A-roll burst: snack vlog moment (different chunk than the post-VO_02 use)
add_video "footage/01_Trip_Setup/003_IMG_0258_road_trip_snacks_no_caffeine.MOV" 30 5
...
finish_montage_with_vo_and_music "audio/voiceovers/VO_01b_pennsylvania_setup.wav" "VO 01b PA setup"

# A-roll burst: waking up reaction
add_video "footage/01_Trip_Setup/005_IMG_0260_waking_up_after_car_nap.MOV" 8 4.5
...
finish_montage_with_vo_and_music "audio/voiceovers/VO_01c_millimeters_payoff.wav" "VO 01c millimeters"
...
add_video "footage/01_Trip_Setup/003_IMG_0258_road_trip_snacks_no_caffeine.MOV" 0 7.5
add_video "footage/01_Trip_Setup/003_IMG_0258_road_trip_snacks_no_caffeine.MOV" 19 6.5

# Nap section a-roll.
add_video "footage/01_Trip_Setup/005_IMG_0260_waking_up_after_car_nap.MOV" 0 7.5
add_video "footage/01_Trip_Setup/006_IMG_0261_car_nap_recovery_drive_resumes.MOV" 0 6.5
add_video "footage/01_Trip_Setup/006_IMG_0261_car_nap_recovery_drive_resumes.MOV" 29 5.5
...
add_video "footage/02_Drive_To_Titusville/007_IMG_0263_morning_highway_update.MOV" 21 11.5
MUSIC_BED="$MUSIC_MORNING"
start_montage
...
add_video "footage/02_Drive_To_Titusville/011_IMG_0267_in_the_woods_almost_there.MOV" 0 13.5
add_video "footage/02_Drive_To_Titusville/012_IMG_0268_double_big_mac_lunch.MOV" 0 5

# David's keyboard world. Rotations applied from review notes. Talking-head
# durations bumped 1.5s for sentence-tail buffer.
add_video "footage/03_David_Factory_Visit/013_IMG_0269_keyboard_21_intro.MOV" 0 8.5
